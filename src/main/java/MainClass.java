...
public class MainClass
{
    public static void main(String[] args)
    {
        ...
        for(Map.Entry<String, Long> group : groupsMap.entrySet())
        {
            System.out.printf("ID: %d NAME: %s \n", group.getValue(),group.getKey());
        }
        */
        System.out.println(Group.groupsMap.get("9ПО-41"));

        System.out.println("Студентов какой группы вывести? ");
        String nameGroup = in.nextLine();
        if(Group.groupsMap.get(nameGroup) != null){
            System.out.println(groupList.get(Math.toIntExact(Group.groupsMap.get(nameGroup)) - 1).toString());
        }
